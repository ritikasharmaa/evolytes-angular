...
@Component({
  selector: 'app-evo-autocomplete-dropdown',
  templateUrl: './evo-autocomplete-dropdown.component.html',
  styleUrls: ['./evo-autocomplete-dropdown.component.css']
})
export class EvoAutocompleteDropdownComponent implements OnInit {
  ...
  ngOnInit() {
    if (this._currentValue) {
      this.currentInput = this._currentValue.value;
    }
  }
  ...
  getList(): { key: string, value: any, iconURL: string }[] {
    if (this.currentInput) {
      const reg = new RegExp(this.currentInput);
      return this.list.filter(element => element.value.match(reg));
    } else {
      return this.list;
    }
  }
